...
void parse_cmdline_hdf2nc(int argc, char *argv[], cmdline *cmd, dir_meta *dm, grid *gd, zfpacc *zfpacc)
{
	...
	OPT_COMPRESS_ZFP, OPT_COMPRESS_SPERR, OPT_COMPRESS_ZFP_LOSSLESS, OPT_COMPRESS_BITGROOM_1, OPT_COMPRESS_BITGROOM_2,
	...
	static struct option long_options[] =
	{
		...
		{"sperr",      optional_argument, 0, OPT_COMPRESS_SPERR},
...
//				printf("cmd->time = %s %12.6f\n",optarg,cmd->time);exit(0);
			...
			case OPT_COMPRESS_ZFP_LOSSLESS:
				...
				break;
			case OPT_COMPRESS_SPERR:
				cmd->sperr = 1;
				cmd->optcount++;
				printf("*** SPERR compression chosen ***\n");
				break;
