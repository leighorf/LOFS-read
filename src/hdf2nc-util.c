...
// ORF temporary Larry kludge: Calculate thv0 here, rather than
// reading from LOFS data (LOFS will save this in future)
//
	for (iz=0; iz<=gd.NZ; iz++) snd->thv0[iz] = snd->th0[iz] * (1.0+reps*snd->qv0[iz])/(1.0+snd->qv0[iz]);
// CM1:		 thv0(i,j,k)=th0(i,j,k)*(1.0+reps*qv0(i,j,k))/(1.0+qv0(i,j,k))
...
//ORF 2022-02-10
//Holy shit this is important, we need to keep very close track
//on all compression settings, both in LOFS data and here.
//
//Should probably have an external file (like the namelist
//file?) that has the values we want.
