...
int main(int argc, char *argv[])
{
	...
	else if (we_are_makevisit)
	{
		if (argc < 2)
		{
			fprintf(stderr,"Usage: %s [topdir] [X0] [Y0] [X1] [Y1] [Z0] [Z1] \n",progname);
...
// Define Cartesian arrays
	...
	status = nc_put_att_text(ncid, xid, "units", strlen("km"), "km");if (status != NC_NOERR) ERROR_STOP("nc_put_att_text failed");
	status = nc_put_att_text(ncid, yid, "units", strlen("km"), "km");if (status != NC_NOERR) ERROR_STOP("nc_put_att_text failed");
	status = nc_put_att_text(ncid, zid, "units", strlen("km"), "km");if (status != NC_NOERR) ERROR_STOP("nc_put_att_text failed");
