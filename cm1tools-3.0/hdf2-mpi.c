...
void hdf2nc(int argc, char *argv[], char *ncbase, int X0, int Y0, int X1, int Y1, int Z0, int Z1, int t0, int t1, int inc);
int hhmmss (int time);
void hdf2v5d();
...
int main(int argc, char *argv[])
{
	...
	if (we_are_hdf2nc) 
	{
		if (argc < 12)
		{
			fprintf(stderr,"Usage: %s [topdir] [ncbase] [X0] [Y0] [X1] [Y1] [Z0] [Z1] [t0] [t1] [increment] [varname1 ... varnameX] \n",progname);
...
// no longer call this - see hdf2.c
//	get_first_hdf_file_name(topdir,timedir[0],nodedir[0],firstfilename); //printf("First filename = %s\n",firstfilename);
	...
	if (we_are_hdf2nc)
	{
		int X0,Y0,X1,Y1,Z0,Z1,t0,t1,inc;
		inc=X0=Y0=X1=Y1=Z0=Z1=t0=t1=0;//shut up compiler
		char ncbase[512];
		hdf2nc(argc,argv,ncbase,X0,Y0,X1,Y1,Z0,Z1,t0,t1,inc);
	}
	else if (we_are_hdf2v5d)
	{
		hdf2v5d();
...
//New for accounting for empty node directories (still needs work
//though)
...
#include <mpi.h>
	...
	int status;
	int ncid,nx_dimid,ny_dimid,nz_dimid,x0id,y0id,z0id,x1id,y1id,z1id;
	int xid,yid,zid;
	int x_dimid,y_dimid,z_dimid;
	int varnameid[MAXVARS];
	int dims[3];
	int mpistatus,myid,numprocs;
	int mytime;
...
//	fprintf(stderr,"Just got in to this subroutine\n");
//	fflush(stderr);
	mpistatus=MPI_Init(&argc,&argv); if (mpistatus != MPI_SUCCESS) printf("We died at MPI_Init\n");
	mpistatus=MPI_Comm_rank(MPI_COMM_WORLD,&myid); if (mpistatus != MPI_SUCCESS) printf("We died at MPI_Comm_rank\n");
	mpistatus=MPI_Comm_size(MPI_COMM_WORLD,&numprocs); if (mpistatus != MPI_SUCCESS) printf("We died at MPI_Comm_size\n");

//	MPI_Barrier(MPI_COMM_WORLD);
	
//	fprintf(stderr,"%i We made it past init\n",myid);
//	fflush(stderr);
//	MPI_Barrier(MPI_COMM_WORLD);
	...
	mytime = t0 + inc*myid;
// Usually things won't divide evenly into 16, so the remaining ranks
// have to idle - this is an appropriate use of goto :)
	if(mytime > t1) goto wait_to_die;

	nvar = argc-12;

//	fprintf(stderr,"%i We made it past atois\n",myid);
//	MPI_Barrier(MPI_COMM_WORLD);
//	fflush(stderr);

	for (i=0; i<nvar; i++)
	{
		strcpy(varname[i],argv[i+12]);
//		fprintf(stderr,"%s \n",varname[i]);
	}
//	fprintf(stderr,"%i We made it past varname\n",myid);
//	fflush(stderr);
//	MPI_Barrier(MPI_COMM_WORLD);
	sprintf(ncfilename,"%s.%05i.nc",ncbase,mytime);

//	fflush(stderr);
//	MPI_Barrier(MPI_COMM_WORLD);
if(myid==0)	fprintf(stderr,"myid = %i inc = %i mytime = %i\n",myid,inc,mytime);
if(myid==0)	fprintf(stderr,"hdf2nc %s %s %i %i %i %i %i %i %i ", topdir, ncbase,X0,Y0,X1,Y1,Z0,Z1,mytime);
if(myid==0)	for (i=0;i<nvar;i++) fprintf(stderr," %s",varname[i]);fprintf(stderr,"\n");
if(myid==0)	fflush(stderr);
//	MPI_Barrier(MPI_COMM_WORLD);
...
//	status = nc_create (ncfilename, NC_CLOBBER|NC_64BIT_OFFSET, &ncid); if (status != NC_NOERR) ERROR_STOP ("nc_create failed");
	...
	status = nc_def_dim (ncid, "x", snx, &nx_dimid); if (status != NC_NOERR) ERROR_STOP("nc_def_dim failed"); 
	status = nc_def_dim (ncid, "y", sny, &ny_dimid); if (status != NC_NOERR) ERROR_STOP("nc_def_dim failed");
	status = nc_def_dim (ncid, "z", snz, &nz_dimid); if (status != NC_NOERR) ERROR_STOP("nc_def_dim failed");
// Define Cartesian arrays
	status = nc_def_var (ncid, "x", NC_FLOAT, 1, &nx_dimid, &xid); if (status != NC_NOERR) ERROR_STOP("nc_def_var failed");
	status = nc_def_var (ncid, "y", NC_FLOAT, 1, &ny_dimid, &yid); if (status != NC_NOERR) ERROR_STOP("nc_def_var failed");
	status = nc_def_var (ncid, "z", NC_FLOAT, 1, &nz_dimid, &zid); if (status != NC_NOERR) ERROR_STOP("nc_def_var failed");

	status = nc_put_att_text(ncid, xid, "units", strlen("units"), "km");if (status != NC_NOERR) ERROR_STOP("nc_put_att_text failed");
	status = nc_put_att_text(ncid, yid, "units", strlen("units"), "km");if (status != NC_NOERR) ERROR_STOP("nc_put_att_text failed");
	status = nc_put_att_text(ncid, zid, "units", strlen("units"), "km");if (status != NC_NOERR) ERROR_STOP("nc_put_att_text failed");
...
// don't bother with compression, not worth it
//                  status=nc_def_var_deflate(ncid, varnameid[ivar], 0, 1, 9); //aggressive compression since not mich redundancy where we are
//		            if (status != NC_NOERR) printf ("Cannot nc_def_var_deflate for var #%i %s\n", ivar, varname[ivar]);
	}
	status = nc_enddef (ncid);
	if (status != NC_NOERR) ERROR_STOP("nc_enddef failed");

      status = nc_put_var_float (ncid,xid,xhout); if (status != NC_NOERR) ERROR_STOP ("nc_put_var_float failed");
      status = nc_put_var_float (ncid,yid,yhout); if (status != NC_NOERR) ERROR_STOP ("nc_put_var_float failed");
      status = nc_put_var_float (ncid,zid,zhout); if (status != NC_NOERR) ERROR_STOP ("nc_put_var_float failed");
	...
	for (ivar = 0; ivar < nvar; ivar++)
	{
		...
		{
			read_hdf_mult_md(ubuffer,topdir,timedir,nodedir,ntimedirs,dn,dirtimes,alltimes,ntottimes,mytime,"uinterp",X0,Y0,X1,Y1,Z0,Z1,nx,ny,nz,nodex,nodey);
			read_hdf_mult_md(vbuffer,topdir,timedir,nodedir,ntimedirs,dn,dirtimes,alltimes,ntottimes,mytime,"vinterp",X0,Y0,X1,Y1,Z0,Z1,nx,ny,nz,nodex,nodey);
		...
		{
			read_hdf_mult_md(buffer,topdir,timedir,nodedir,ntimedirs,dn,dirtimes,alltimes,ntottimes,mytime,varname[ivar],X0,Y0,X1,Y1,Z0,Z1,nx,ny,nz,nodex,nodey);
			status = nc_put_vara_float (ncid, varnameid[ivar], start, edges, buffer);
		}
		if (status != NC_NOERR) 
		{
			fprintf(stderr,"Could not write variable %s at time %i to %s\n", varname[ivar],mytime,ncfilename);
